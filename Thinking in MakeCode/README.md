...
至此，我们似乎陷入了一种思维困境，再继续使用延时的方法，每一段积木（代码）都在等待前面积木（代码）执行完毕，且每当有需要延时的地方就要再次统筹前面的延时时间，如此往复，我们的代码将陷入更加复杂而容易出错的困境中，每一行就像是互相之间锁死在一起，我们可以称其为“blocking code”阻塞代码，而这样的编程思维在程序的复杂性不断增加的过程中造成的灾难性后果将会变得愈发严重而难以挽救。  
...
现在，我们再回顾1.2小节，使一个LED间隔1.5秒闪烁，同时使另一个LED间隔2.5秒闪烁，以前文中所描述的方法，仅仅需要改动While循环中if的判断条件即可，即便是我们再增加其他的需求，比如说要以不同频率闪烁更多LED，我们的头脑中也已经可以直接产生非常清晰的设计思维，且不会因为需求的增加或变更而牵连程序中其余部分，程序会按照我们设计的顺序与设定的时间执行每一个功能，这就是“non-blocking”非阻塞程序设计思维。
